...
/// </summary>
/// <remarks>
/// The warm-cache write guard's signal: a generation worker holding a cached CEK can outlive
/// the key's destruction by the cache's MaxAge, and without the destroyed-key write guard in
/// the statement write repository it would publish a statement that becomes permanently
/// unreadable the moment the cache expires. SCHEDULED_DESTRUCTION counts too — writing new
/// statements into a cooling-off window creates data that is about to become unreadable, which
/// is worse than refusing. The run item that hits this must fail TERMINALLY: the condition is
/// deterministic and no retry changes it.
/// </remarks>
